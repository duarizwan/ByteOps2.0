...
Spawns the Gmail MCP server as a stdio subprocess (one per request),
...
Key design decisions:
  - No module-level globals or monkey-patching — the SSE queue and
    ToolConnection are passed in as parameters.
  - All credentials (access_token, refresh_token, client_id, secret) are
...
async def run_gmail_agent(
    ...
    env["GMAIL_ACCESS_TOKEN"] = tool_connection.access_token
    if tool_connection.refresh_token:
        env["GMAIL_REFRESH_TOKEN"] = tool_connection.refresh_token
    env["GMAIL_CLIENT_ID"] = settings.gmail_client_id
    env["GMAIL_CLIENT_SECRET"] = settings.gmail_client_secret

    server_params = StdioServerParameters(
        command=sys.executable,
        args=["-m", "app.mcp_servers.gmail.server"],
        env=env,
    )
    ...
    try:
        ...
        # Recursively unwrap nested anyio/MCP TaskGroup exception groups
